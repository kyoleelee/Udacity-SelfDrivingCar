    ...
    if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          double psi = j[1]["psi"];
          double v = j[1]["speed"];
					double steer_angle = j[1]["steering_angle"];  // opposite direction in simulator
          ...
          */
          ...
          psi = 0 - (v * steer_angle * latency / Lf);   	// psi:  psi0 = 0, due to the car coordinate system
          v = v + acceleration * latency;
          double cte = coeffs[0] + (v * sin(-atan(coeffs[1])) * latency);
		  		double epsi = -atan(coeffs[1]) - (v * steer_angle * latency / Lf);
